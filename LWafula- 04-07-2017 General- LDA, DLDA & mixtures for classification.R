#Lwafula
# this just uses matrix results from the LWafula-General- LDA,DLDA & mixtures scripts
...
#boxplots for untransformed vs transformed cases
plot(value~as.factor(gr),data=results,xlab=expression(paste(lambda,' ','value')),ylab=expression(paste(T^2,' ','value')),pch=19,cex=0.5)
...
#plots
#due to high outliers, plot of max 500?
hi<-paste0('report/dataView/manova plots/individual lambdas/hotellings500max_allactiveTRTS_Boxplot_all_lambdas','.pdf')
pdf(hi)
plot(value~as.factor(gr),data=results,xlab=expression(paste(lambda,' ','value')),ylab=expression(paste(T^2,' ','value')),pch=19,
     cex=0.25,ylim=c(-70,500))
...
#all
hi<-paste0('report/dataView/manova plots/individual lambdas/hotellings_allactiveTRTS_Boxplot_all_lambdas','.pdf')
pdf(hi)
plot(value~as.factor(gr),data=results,xlab=expression(paste(lambda,' ','value')),ylab=expression(paste(T^2,' ','value')),pch=19,
...
# logs ?
#due to high outliers, plot of max 500?
hi<-paste0('report/dataView/manova plots/individual lambdas/loghotellings500max_allactiveTRTS_Boxplot_all_lambdas','.pdf')
pdf(hi)
plot(log(value)~as.factor(gr),data=results,xlab=expression(paste(lambda,' ','value')),ylab=expression(paste(T^2,' ','value')),pch=19,
     cex=0.25,ylim=c(log(70),log(500)))
...
#all
hi<-paste0('report/dataView/manova plots/individual lambdas/loghotellings_allactiveTRTS_Boxplot_all_lambdas','.pdf')
pdf(hi)
plot(log(value)~as.factor(gr),data=results,xlab=expression(paste(lambda,' ','value')),ylab='',pch=19,
     cex=0.25,ylim=c(-log(7000),log(max(results$value))), cex.main=0.75,cex.lab=0.85)
