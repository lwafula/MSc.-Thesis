...
# cell level
fileb1<-file.path(datawdtranslambda,paste0('p1601xxPPS_means_preprocessedfeat_act_',lambdas[i],'.RData'))
...
mpya<-rbind(mpya,mpya.1)
  ...
  #well level 
  fileb1<-file.path(datawdtranslambda,paste0('aggregated_p1601xxPPS_means_preprocessedfeat_act_',lambdas[i],'.RData'))
  load(fileb1)
  datpopt.featurelambda<-glogdatp[glogdatp[['Active_FractionOfRepl']]>=0.5 ,]
  rm(list=c('glogdatp','fileb1'))
...
auc<-paste0('aggregated_auc',lambv[i],'.pdf')
