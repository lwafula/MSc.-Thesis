...
for(i in 1:dim(prove.data)[1]){
  ...
  #plot
  nameplot<-paste0('graphs/diff250000/lambda',lambdavals,'combined_diff_250000_trt_combn',i,'.pdf')
...
########################## the lambda 105 data was created here, also has graphs where difference was from min T2 to maxT2 ####################################
...
# 707 treatments, top 10 features;  saved data ready for this comparison
load('data/p1601xxPPS_means_preprocessed.Rdata')
datpOpt.feature<-datp[datp[['Active_FractionOfRepl']]>=0.5 ,c("Treatment", attr(datp, "features_mRMR_optN")[1:10])]

ttrt_namedID<-'July2017results_H2_allTrts10fts_untransformed.csv'; ttrt_namedID<-file.path(datawdmatrixpvals,ttrt_namedID)
ttrt_namedID<-read.table(ttrt_namedID,sep=',',header=T)
ttrt_namedID<-ttrt_namedID[,-1]
ttrt_namedID$gr<-'0'

# lambda 10.5 Hoteling's , not yet saved
file.names <- dir(datawdtranslambda, pattern ="p1601xxPPS_means_preprocessedfeat_act_")
i=5 #check that this refers to lambda=10.5
...
for (j in 1:length(treatkp)) {
  for (i in 1:length(treatkp)) {
    if(j<i) { #you may as well do for (j!=i) though it is just a mirror-image
      ...
      resultsname[j,i]<-fit$stats$statistic
    }
...
tr<-'data/transformed_lambda10_5.csv'
tr<- file.path(prov.manenoz,tr)
write.csv(resultsname10_5,tr)
rm(resultsname)
#

# merge the two results
merged<-merge(ttrt_namedID,resultsname10_5,by=c('Var1','Var2'))
hi<-paste0('report/dataView/manova plots/proveOfTransformationWork/graphs/hotellings_untransformed_vs',lambdaval,'.pdf')
pdf(hi)
...
# View( merged[merged[['value.y']]==max(merged[['value.y']])|merged[['value.x']]==max(merged[['value.x']]),]); ones that look like outlying
...
trans_minT2_to_maxT2=
  ggplot(data = datp_provemin2max,aes(x = feature, y = value,group=interaction(indexreplicates,Treatment),
              col=Treatment)) + geom_point(pch=19,cex=0.55) + theme_bw() + geom_line(lwd=0.6)+ theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  xlab('feature')+ylab('replicate value')+ guides(fill=guide_legend(ncol=2)) + 
  theme(legend.position="top",legend.text=element_text(size=10)) 
...
trans_minT2_to_maxT2_transformed=
  ggplot(data = glogdatp_provemin2max,aes(x = feature, y = value,group=interaction(indexreplicates,Treatment),col=Treatment)) + 
