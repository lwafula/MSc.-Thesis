...
\centering
\includegraphics[height=8.5cm,width=9cm]{data1_distr_overlaps_pre_post_transformation.pdf}
\end{frame}
...
library(data.table)
library(graphics)
lambdafeatactv_alphacorrected<-read.csv(
  'E:/uhasselt/22yrsem/MThesis/data/glog transformed datasets by lambda/lambdafeatactv_alphacorrected.csv')
lambdafeatactv_alphacorrected<-lambdafeatactv_alphacorrected[,-1]

pdf('E:/uhasselt/22yrsem/MThesis/report/finalreport/optsfeat_active.pdf')
par(mar = c(5,5,2,5))
with(lambdafeatactv_alphacorrected, plot(lambdafeatactv_alphacorrected[,1],lambdafeatactv_alphacorrected[,3],
                                         xlab=expression(lambda),ylab='active calling %',ylim=c(55,59), lty=1,lwd=2,cex=0.5,type='l'))
abline(h=56.42458,lty=2,col='black',lwd=2)
## add line for number of features selected
par(new = T)
with(lambdafeatactv_alphacorrected, plot(lambdafeatactv_alphacorrected[,1],lambdafeatactv_alphacorrected[,2],pch=19,
                                         axes=F, xlab=NA, ylab=NA, cex=0.6,type='l',col='blue',lwd=1))
abline(h=19,lty=2,col='blue',lwd=1)
axis(side = 4,col='blue')
mtext(side = 4, line = 3,eval(bquote(expression(paste('Optimal features selected')))),col='blue')
legend('topright',legend=c(expression(paste('transformed')),'% (untransformed)'),
