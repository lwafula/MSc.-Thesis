...
# 2. includes results from lambdas [0.5 T2 statistics compared to untransformed -with related treatments identified]
rm(list=ls())
...
#due to high outliers, plot of max 500?
  hi<-paste0('report/dataView/manova plots/individual lambdas/D2_hotellings500max_allactiveTRTS_Boxplot_all_lambdas','.pdf')
  pdf(hi)
...
# logs ?
#due to high outliers, plot of max 500?
hi<-paste0('report/dataView/manova plots/individual lambdas/D2_loghotellings500max_allactiveTRTS_Boxplot_all_lambdas','.pdf')
pdf(hi)
plot(log(value)~as.factor(gr),data=results,xlab=expression(paste(lambda,' ','value')),ylab=expression(paste(T^2,' ','value')),pch=19,
     cex=0.25,ylim=c(log(10),log(500)))
