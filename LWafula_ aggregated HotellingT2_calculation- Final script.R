...
file.names <- dir(datawdtranslambda, pattern ="aggregated_p1601xxPPS_means_preprocessedfeat_act_")
for (i in 1:length(file.names)){
  ...
  lambdaval=   gsub("aggregated_p1601xxPPS_means_preprocessedfeat_act_","",file.names[i]); lambdaval<-gsub(".RData","",lambdaval); lambdaval
  ...
  h2<-paste0('report/dataView/manova plots/aggregated_hotellings_allactiveTRTS_lambda=',lambdaval,'.pdf')
...
for (i in 1:length(lambdas)){
  ...
  #plots
  hi<-paste0('report/dataView/manova plots/individual lambdas/aggregated_hotellings_allactiveTRTS_lambda=',lambv[i],'.pdf')
  pdf(hi)
...
for(i in 1:length(lambdas)){
  ...
  # plot
  # #add here plots for other transformations for the report and H2 comparisons btwn untransformed and transformed
  hi<-paste0('report/dataView/manova plots/individual lambdas/logs/aggregated_LOG_hotellings_allactiveTRTS_lambda=',lambdas[i],'.pdf')
  pdf(hi)
  
  plot(mydensitylog[mydensitylog$gr==gr[1],]$y~mydensitylog[mydensitylog$gr==gr[1],]$x,type='l',lwd=1,lty=1,ylim=c(0,2.5),xlim=c(0,14),
...
for(i in 1:length(lambdas)){
  ...
  h2temp<-read.table(h2temp,header = T,sep=',')
  h2temp<-h2temp[,-1]
  h2temp<-melt(h2temp); h2temp<-h2temp[complete.cases(h2temp$value),]
  lambv[i]<-lambdas[i];lambv[i]
  ...
  hi<-paste0('report/dataView/manova plots/individual lambdas/aggregated_pvalue_hotellings_allactiveTRTS_lambda=',lambv[i],'.pdf')
  pdf(hi)
  ...
  legend(max(length(merge[merge[['grp']]=='untransformed',][['value']]),length(merge[merge[['grp']]==paste('lambda',lambv[i]),][['value']]))+
           0.1*(max(length(merge[merge[['grp']]=='untransformed',][['value']]),length(merge[merge[['grp']]==paste('lambda',lambv[i]),][['value']]))),0.8, 
