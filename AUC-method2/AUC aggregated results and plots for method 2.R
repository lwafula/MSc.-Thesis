...
for (i in 1:length(lambdas)){
  ...
  # cell level
  fileb1<-file.path(datawdtranslambda,paste0('p1602xxPPS_means_preprocessedfeat_act_',lambdas[i],'.RData'))
  ...
  #well level 
  fileb1<-file.path(datawdtranslambda,paste0('aggregated_p1602xxPPS_means_preprocessedfeat_act_',lambdas[i],'.RData'))
  load(fileb1)
  datpopt.featurelambda<-glogdatp[glogdatp[['Active_FractionOfRepl']]>=0.5 ,]
  rm(list=c('glogdatp','fileb1'))
  ...
  auc<-paste0('D2_aggregated_auc',lambv[i],'.pdf')
  auc<-file.path(datawdauc,auc)
  pdf(auc)
  plot(value~as.factor(gr),data=mpya,xlab='',ylab='AUC of replicates vs non-replicates across runs',
ylim=c(0.94,0.97),main='Evolution of AUC assessing replicability Vs transformation parameter',pch=19)
