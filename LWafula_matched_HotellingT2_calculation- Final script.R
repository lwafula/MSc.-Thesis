...
#DATASET 2
### comparison for Hotelling's T2 for only treatments that were active pre- and post-transformation [using lambda= 0.5]
...
file.names <- dir(datawdtranslambda, pattern ="^p1602xxPPS_means_preprocessedfeat_act_0.5")
...
intersect(names(table(datpOpt.feature$Treatment)),names(table(datpopt.featurelambda$Treatment)) ) # 642
...
#lambda value 
lambdaval=   gsub("p1602xxPPS_means_preprocessedfeat_act_","",file.names[1]); lambdaval<-gsub(".RData","",lambdaval); lambdaval
...
#plots
hi<-paste0('report/dataView/manova plots/individual lambdas/matched_hotellings_allactiveTRTS_lambda=',lambv[1],'.pdf')
pdf(hi)

plot(mydensity[mydensity$gr==gr[1],]$y~mydensity[mydensity$gr==gr[1],]$x,type='l',lwd=1,lty=2,ylim=c(0,80e+05),
     col='black',xlab=expression(paste('T'^2,' ','value')),ylab='Density', xlim=c(0,max(mydensity$x)),
     main=expression(paste('Distribution of',' ','T'^2,' ','matched treatments')))
...
#keep only Hotelling's results for now
file.names<-file.names[!grepl('^(matched_results_pvalue0.5)',file.names, ig = TRUE)]
...
plot(mydensitylog[mydensitylog$gr==gr[1],]$y~mydensitylog[mydensitylog$gr==gr[1],]$x,type='l',lwd=2,lty=1,ylim=c(0,1),xlim=c(0,14),
...
# for (i in 1:length(file.names)){
h2temp<-file.path(datawdmatrixpvals,file.names[2])
lambv[1]<-gsub('matched_results_pvalue','',file.names[2]); lambv[1]<-gsub('.csv','',lambv[1]); lambv[1]
h2temp<-read.table(h2temp,header = T,sep=',')
h2temp<-h2temp[,-1]
...
hi<-paste0('report/dataView/manova plots/individual lambdas/D2_matched_pvalue_hotellings_allactiveTRTS_lambda=',lambv[1],'.pdf')
pdf(hi)
...
abline(h=0.05/(642*641*0.5),col='blue',lwd=2,lty=1)
...
# c(unt_pval5*642*641*0.5*0.01,t_pval5*642*641*0.5*0.01,unt_pval5*642*641*0.5*0.01-t_pval5*642*641*0.5*0.01) #c(305.1029-260.8514,44.2515)

# proportions after correcting for multiplicity (Bonferroni) 128370.1727 129197.3319   -827.1592 [b4=62.388%,after= 62.79%]
unt_pval5<-round(length(merge[merge[['grp']]=='untransformed' & merge[['value']]>(0.05/(642*641*0.5)),][['value']])*100/
                   length(merge[merge[['grp']]=='untransformed',][['value']]),3)
t_pval5<-round(length(merge[merge[['grp']]==paste('lambda',lambv[1]) & merge[['value']]>(0.05/(642*641*0.5)),][['value']])*100/
                 length(merge[merge[['grp']]==paste('lambda',lambv[1]),][['value']]),3)
c(unt_pval5*642*641*0.5*0.01,t_pval5*642*641*0.5*0.01,unt_pval5*642*641*0.5*0.01-t_pval5*642*641*0.5*0.01) 

#add legend with thz information about %
legend(max(length(merge[merge[['grp']]=='untransformed',][['value']]),length(merge[merge[['grp']]=='lambda 0.5',][['value']]))+
       0.035*max(length(merge[merge[['grp']]=='untransformed',][['value']]),length(merge[merge[['grp']]=='lambda 0.5',][['value']])),0.8, 
       legend = c(
...
#DATASET 2
### comparison for Hotelling's T2 for only treatments that were active pre- and post-transformation [using lambda= 0.1]
...
# the transformed data
file.names <- dir(datawdtranslambda, pattern ="^p1602xxPPS_means_preprocessedfeat_act_0.1")
fileb1<-file.path(datawdtranslambda,file.names)
load(fileb1)
datpopt.featurelambda<-glogdatp[glogdatp[['Active_FractionOfRepl']]>=0.5 ,c("Treatment", attr(glogdatp, "features_mRMR_optN")[1:10])]
rm(list=c('glogdatp','fileb1'))
...
# for both, just keep those treatments that were active in both untransformed and transformed data sets
intersect(names(table(datpOpt.feature$Treatment)),names(table(datpopt.featurelambda$Treatment)) ) # 642 [0.5] & 649 [0.1]
...
results_H2_allTrts10fts_untransformed<-'report/results-matricesH2_pvalues etc/matched_Aug2017results_H2_allTrts10fts_untransformed_0.1.csv'
write.csv(results,results_H2_allTrts10fts_untransformed)
...
pdf('report/dataView/manova plots/matched_hotellings_allactiveTRTS_untransformed0point1.pdf')
plot(myhist,xlab=expression(paste('T'^2,' ','value')),ylab='counts',xlim=c(0,350000),main=paste('calculated Hotelling',"'",'s statistic- Untransformed data'),
     ylim=c(0,1000000))
lines(mydensity,lwd=2)
dev.off()
...
results_H2pvalues_allTrts10fts_untransformed<-
  'report/results-matricesH2_pvalues etc/matched_Aug2017results_H2pvalues_allTrts10fts_untransformed0.1.csv'
write.csv(results_pvalue,results_H2pvalues_allTrts10fts_untransformed)
...
# add results for lambda= 0.1   # distribution of the Hoteling's T2 statistic and its p-value
treatkp<-names(table(datpopt.featurelambda$Treatment))
#lambda value 
lambdaval=   gsub("p1602xxPPS_means_preprocessedfeat_act_","",file.names[1]); lambdaval<-gsub(".RData","",lambdaval); lambdaval
resultsname<-paste0('results',lambdaval)
...
# 17-08-2017: ALL THE ABOVE ARE ALREADY SAVED [only for pvalues-the pvalues corrected for lambda= 0.1]
# Hotelling's distribution (649*648*0.5)

# P-VALUES

results_H2pvalues_allTrts10fts_untransformed<-'matched_Aug2017results_H2pvalues_allTrts10fts_untransformed0.1.csv'
results_H2pvalues_allTrts10fts_untransformed<-file.path(datawdmatrixpvals,results_H2pvalues_allTrts10fts_untransformed)
results_H2pvalues_allTrts10fts_untransformed<-read.table(results_H2pvalues_allTrts10fts_untransformed,header=T,sep=',')
results_H2pvalues_allTrts10fts_untransformed<-results_H2pvalues_allTrts10fts_untransformed[,-1]
...
# individual pvalues
file.names <- dir(datawdmatrixpvals, pattern ="matched_results")
#keep only Hotelling's pvalues results for now [lambda= 0.1]
file.names<-file.names[grepl('^(matched_results_pvalue0.1)',file.names, ig = TRUE)]
lambv<-c()
...
plot(merge[['value']],type='n',xlab='index',ylab='p-value',xlim=c(0,max(length(merge[merge[['grp']]=='untransformed',][['value']]),
                                                                        length(merge[merge[['grp']]==paste('lambda',lambv[1]),][['value']]))))
abline(h=0.05/(649*648*0.5),col='blue',lwd=2,lty=1)
points(merge[merge[['grp']]=='untransformed',][['value']],pch=19,col='black',cex=0.65)
points(merge[merge[['grp']]==paste('lambda',lambv[1]),][['value']],pch=19,col='red',cex=0.65)

# logs? didn't gv anything good enough

# proportions after correcting for multiplicity (Bonferroni) (131628.570 132040.711   -412.141) [b4= 62.598%,after= 62.794%]
unt_pval5<-round(length(merge[merge[['grp']]=='untransformed' & merge[['value']]>(0.05/(649*648*0.5)),][['value']])*100/
                   length(merge[merge[['grp']]=='untransformed',][['value']]),3)
t_pval5<-round(length(merge[merge[['grp']]==paste('lambda',lambv[1]) & merge[['value']]>(0.05/(649*648*0.5)),][['value']])*100/
                 length(merge[merge[['grp']]==paste('lambda',lambv[1]),][['value']]),3)
c(unt_pval5*649*648*0.5*0.01,t_pval5*649*648*0.5*0.01,unt_pval5*649*648*0.5*0.01-t_pval5*649*648*0.5*0.01) 
