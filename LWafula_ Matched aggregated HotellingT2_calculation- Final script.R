...
# wellvs cell vs untransformd for lambda =10.5 and 11
...
cellwell<-'E:/uhasselt/22yrsem/MThesis/report/results-matricesH2_pvalues etc/cellwell'
lambv<-c(8.5,10,10.5,11)
for (i in 1:length(lambv)){
#                                         untransformed
...
#plots
hi<-file.path(cellwell,paste0('graphs/matched_aggregated_hotellings_allactiveTRTS_lambda=',lambv[i],'.pdf'))
pdf(hi)
...
# plot
hi<-file.path(cellwell,paste0('graphs/LOG_matched_aggregated_hotellings_allactiveTRTS_lambda=',lambv[i],'.pdf'))
pdf(hi)
...
hi<-file.path(cellwell,paste0('graphs/LOG_matched_aggregated_Boxplothotellings_allactiveTRTS_lambda=',lambv[i],'.pdf'))
pdf(hi)
