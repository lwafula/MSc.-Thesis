...
dataglog_data<-'E:/uhasselt/22yrsem/MThesis/data/glog transformed datasets by lambda'
...
########## ``````````````````````````````````````````
load(file.path(dataglog,'aggregated_batch1.RData'))
# 
dw <- data.table(dw) 
...
#
lambda=c(0.5,8.5,10,10.5,11,11.5)
...
fileb1<-data.frame(dw)
  ...
  fn_out<- paste0('aggregated_p1601xxPPS_means_preprocessedfeat_act_',lambda[j],'.RData')
...
# 
# #save
lambdafeatactv_alphacorrected0<-'E:/uhasselt/22yrsem/MThesis/data/glog transformed datasets by lambda/aggregated_lambdafeatactv_alphacorrected.csv'
