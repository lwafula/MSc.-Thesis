# DATASET 2
# NOTE: ALWAYS MAKE SURE THE WELL-DATA AND SINGLE-CELL BATCH DATA YOU ARE TRYING TO CONNECT ARE CONCURRENT
...
# -then tranform using proposal lambdas
...
setwd('E:/uhasselt/22yrsem/MThesis/DATASET2/data')
library(hexbin)
library(ggplot2)
library(matlab)
library(plyr)
...
# add plate layout to know the controls, always check that this data here is in line with the batches u r trying to connect
...
########## ``````````````````````````````````````````
load(file.path(dataglog,'aggregated_batch2.RData'))
# 
dw <- data.table(dw) 
...
# 0.5 and 8.5,10 represent lambdas that were performing poorly in the single cell data, while 10.5,11 and 11.5 were performing nicely
#
lambda=c(0.1,0.5,8.5,10.5)
lambdafeatactv_alphacorrected=array(NA,dim=c(length(lambda),3))

for(j in 1:length(lambda)){
  fileb1<-data.frame(dw)
  # already at aggregate level for dataset1, adjust here for lambda values
  ...
  datadir<-'E:/uhasselt/22yrsem/MThesis/DATASET2/data'
  dataview<-'E:/uhasselt/22yrsem/MThesis/DATASET2/report/dataView'
  # # combined data for labda=j 
  str(fileb1, list.len=ncol(fileb1))
  
  ## ``` Add plate layout (compound, concentration, .) ````
  
  glogdatp <- 
    merge(fileb1, winfo, by = c('WELL_ID', 'PLATE_ID','COL_NR','ROW_NR'), 
  ...
  ## appropriate.
  ## % active treatments:  58.10056 meaning that 58.10056% of the CPDs have a significant different to DMSO
  ...
  #save ; note that this was initially saved wrongly as 1602 refering to data2
  fn_out<- paste0('aggregated_p1602xxPPS_means_preprocessedfeat_act_',lambda[j],'.RData')
...
# 
# #save
lambdafeatactv_alphacorrected0<-'E:/uhasselt/22yrsem/MThesis/DATASET2/data/glog transformed datasets by lambda/aggregated_lambdafeatactv_alphacorrected.csv'
