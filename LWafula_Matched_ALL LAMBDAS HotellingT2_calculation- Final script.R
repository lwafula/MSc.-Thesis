...
#lambda value 
lambdaval=gsub("p1601xxPPS_means_preprocessedfeat_act_","",file.names[k]); lambdaval<-gsub(".RData","",lambdaval); lambdaval
...
datasave<-'report/results-matricesH2_pvalues etc/matchedactivetreatments/data1Boxplots.RData'
...
#plots
hi<-file.path(matched,paste0('graphs/matched_hotellings_allactiveTRTS_lambda=',lambv[i],'.pdf'))
pdf(hi)
...
#plots
hi<-file.path(matched,paste0('graphs/clipped_matched_hotellings_allactiveTRTS_lambda=',lambv[i],'.pdf'))
pdf(hi)
...
# plot
hi<-file.path(matched,paste0('graphs/LOG_matched__hotellings_allactiveTRTS_lambda=',lambv[i],'.pdf'))
pdf(hi)

plot(mydensitylog[mydensitylog$gr==gr[1],]$y~mydensitylog[mydensitylog$gr==gr[1],]$x,type='l',lwd=2,lty=1,ylim=c(0,2.5),xlim=c(0,14),
...
hi<-file.path(matched,paste0('graphs/LOG_matched_Boxplothotellings_allactiveTRTS_lambda=',lambv[i],'.pdf'))
pdf(hi)
