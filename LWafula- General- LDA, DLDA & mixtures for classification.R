...
plot(myhist,xlab=expression(paste('T'^2,' ','value')),ylab='counts',xlim=c(0,22000),
     main=paste('calculated Hotelling',"'",'s statistic- Untransformed data'),ylim=c(0,1500000))
...
for (i in 1:length(file.names)){
  ...
  imageplot<-paste0('report/dataView/manova plots/D2_image lambda=',lambdaval,'.pdf')
  ...
  plot(myhist,xlab=expression(paste('T'^2,' ','value')),ylab='counts',xlim=c(0,10000),
       main=paste('calculated Hotelling',"'",'s statistic- transformed data'),ylim=c(0,2000000))
  lines(mydensity,lwd=2)
  dev.off()
...
# 08-07-2017: ALL THE ABOVE ARE ALREADY SAVED
...
mydensity$gr<-'0'
plot(mydensity$y~mydensity$x,type='l',lwd=2.5,lty=2,ylim=c(0,80e+05),
...
for(j in 1:length(gr)){
  ...
  legend(10000,(300e+05)*0.015*gr[j],legend = gr[j],col=j+1,lty=1,bty = 'n',cex=0.5,lwd=2)
}
dev.off()
