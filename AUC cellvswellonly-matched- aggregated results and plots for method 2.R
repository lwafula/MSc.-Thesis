...
# aggregated AUC
# using only those actively-called treatments that appear in cell- and well-level transformed data
rm(list=ls())
...
for(i in 1:length(lambv)){
  ...
  int.all<-Reduce(intersect, list(names(table(datpopt.featurelambda$Treatment)),
                                  names(table(welldatpopt.featurelambda$Treatment)))) # 696 for lambda= 10.5
  ...
  FSres.cellfeaturelambda <- evaluationAUC(
  ...
  mpya<-melt(FSres.cellfeaturelambda)
  ...
  auc<-paste0('matched_cellwell/cellvswell_aggregated_auc',lambv[i],'.pdf')
  ...
  abline(h=median(FSres.cellfeaturelambda$aucAll),col='red',lwd=2,lty=2)
  legend('bottomleft',legend=c(expression(paste('median for cell-level'))),lty=c(2),col=c('red'),lwd=c(2),bty='n')
  dev.off()
}
