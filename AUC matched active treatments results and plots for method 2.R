...
for(i in 1:length(lambv)){
  ...
  int.all<-Reduce(intersect, list(names(table(datp.orig$Treatment)),names(table(datpopt.featurelambda$Treatment)))) # 683 for lambda= 10.5
 ...
 #                    save data
 dsave<-file.path(datawdauc,paste0('matched_cell/data1allauc.RData'))
 ...
 pdf<-file.path(datawdauc,paste0('matched_cell/matched_auc_all_lambdas.pdf'))
 pdf(pdf)
