...
# 639 treatments, top 10 features
...
i=1 #check that this refers to lambda=0.5
...
resultsname0_5<-resultsname

#save
tr<-'data/transformed_lambda0_5.csv'
...
#
# into one line for plotting
resultsname0_5<-melt(resultsname0_5)
resultsname0_5<-resultsname0_5[complete.cases(resultsname0_5[['value']]),]
resultsname0_5$gr<-'0.5'

# merge the two results
merged<-merge(ttrt_namedID,resultsname0_5,by=c('Var1','Var2'))
hi<-paste0('report/dataView/manova plots/individual lambdas/D2_hotellings_untransformed_vs',lambdaval,'.pdf')
pdf(hi)
plot(merged[['value.x']],merged[['value.y']],pch=19,col='black',cex=0.65,
     xlab=expression(paste('untransformed', ' ', T^2,' ','value')),ylab='')

title(ylab=expression(paste('transformed',' ','(',lambda,' ','= 0.5',')', ' ', T^2,' ','value')),family='',line = 2.5)
dev.off()
...
datp_provemin2max=
  data.table(subset(datp.prove, datp.prove$Treatment %in% trts[c(1,3)]))
...
# rename TRT levels
datp_provemin2max$Treatment=  gsub("in Hepa-ER-Mito","",datp_provemin2max$Treatment)
...
trans_minT2_to_maxT2=
  ggplot(data = datp_provemin2max,aes(x = feature, y = value, group=interaction(indexreplicates,Treatment),col=Treatment)) + 
  geom_point(pch=19, cex=0.55) + theme_bw() + geom_line(lwd=0.6)+ theme(axis.text.x = element_text(angle = 90, hjust = 1))+
...
glogdatp_provemin2max=
  data.table(subset(glogdatp.prove, glogdatp.prove$Treatment %in% trts[c(1,3)]))
...
# rename TRT levels
glogdatp_provemin2max$Treatment=  gsub("in Hepa-ER-Mito","",glogdatp_provemin2max$Treatment)
...
datp_provemax2min=
  data.table(subset(datp.prove, datp.prove$Treatment %in% trts[2:3]))
...
# rename TRT levels
datp_provemax2min$Treatment=  gsub("in Hepa-ER-Mito","",datp_provemax2min$Treatment)
...
glogdatp_provemax2min=
  data.table(subset(glogdatp.prove0, glogdatp.prove0$Treatment %in% trts[2:3]))
...
# rename TRT levels
glogdatp_provemax2min$Treatment=  gsub("in Hepa-ER-Mito","",glogdatp_provemax2min$Treatment)
...
#plot
hicomb1<-
  ggplot(data = datacomb0,aes(x = feature, y = value,
                              group=interaction(indexreplicates,Treatment),col=Treatment)) + geom_point(pch=19,
                                                                                                        cex=0.55) + theme_bw() + geom_line(lwd=0.6)+ theme(axis.text.x = element_text(angle = 90, hjust = 1,size=5,color="black"))+
  xlab('feature')+ylab('replicate value')+ guides(fill=guide_legend(ncol=2)) + 
  theme(legend.position="top",legend.text=element_text(size=10))+
...
tr<-'data/transformed_lambda0_5.csv'
tr<- file.path(prov.manenoz,tr)
resultsname0_5<-read.csv(tr,sep=',',header = T)
resultsname0_5<-resultsname0_5[,-1]
#
...
load('data/glog transformed datasets by lambda - Copy/p1602xxPPS_means_preprocessedfeat_act_0.5.Rdata')
lambdavals= 0.5
...
for(i in 1:dim(prove.data)[1]){
  ...
  plots<-
    ggplot(data = datacomb,aes(x = feature, y = value,group=interaction(indexreplicates,Treatment),col=Treatment)) +
    geom_point(pch=19,cex=0.55) + theme_bw() + geom_line(lwd=0.6)+ 
    theme(axis.text.x = element_text(angle = 90, hjust = 1,size=2,color="black"))+
    xlab('feature')+ylab('replicate value')+ guides(fill=guide_legend(ncol=2)) + 
    theme(legend.position="top",legend.text=element_text(size=10))+
...
########### WHERE DIFFERENCE IN THE HOTELLING'S T2 IS GREATER THAN 20000 ###########################################
...
prove.data<-merged[merged[['value.y']]-merged[['value.x']]>20000,]
...
for(i in 1:dim(prove.data)[1]){
  ...
  datacomb$gr <- factor(datacomb$gr, levels=c('0','1'),labels=c('untransformed','transformed'))
  #plot
  nameplot<-paste0('graphs/diff20000/lambda',lambdavals,'combined_diff_2000_trt_combn',i,'.pdf')
  nameplot<-file.path(prov.manenoz,nameplot)
