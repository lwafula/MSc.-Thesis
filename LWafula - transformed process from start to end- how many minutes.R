...
for(i in 1:length(file.names)){
  ...
  load(fileb1)
  # lambda adjusts
  fileb1[,features0]<-log(fileb1[,features0]+sqrt(fileb1[,features0]^2 +0.1)); fileb1<-data.table(fileb1)
...
dim(table(glogdatp[glogdatp$Active_FractionOfRepl>0.5,]$Treatment))*100/1253
