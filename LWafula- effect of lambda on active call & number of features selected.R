...
datawd<-'E:/uhasselt/22yrsem/MThesis/DATASET2/data/glog transformed datasets by lambda'
...
# #plots
lambdavsoptfeature_alpha<-'report/dataView/D2_lambdavsoptfeature_alpha.pdf'
...
plot(lambdafeatactv_alphacorrected[,1],lambdafeatactv_alphacorrected[,2],pch=19,type='l',xlab=expression(lambda),
     ylab='Optimal features',ylim=c(10,50),lwd=1,lty=1)
abline(h=34,lty=2,col='black',lwd=2)
legend(5.5,15,legend=c(expression(paste('transformed by',' ',lambda,' ','= x-value')),'untransformed'),lty=c(1,2),col=c('black','black'),
       lwd=c(1,2),bty='n')
dev.off()
# 
lambdavsactcall_alpha<-'report/dataView/D2_lambdavsactcall_alpha.pdf'
...
plot(lambdafeatactv_alphacorrected[,1],lambdafeatactv_alphacorrected[,3],pch=19,type='l',xlab=expression(lambda),
     ylab='active calling %',ylim=c(50,55),lwd=1,lty=1)
abline(h=51.23703,lty=2,col='black',lwd=2)
legend(5.5,51,legend=c(expression(paste('transformed by',' ',lambda,' ','= x-value')),'untransformed'),
